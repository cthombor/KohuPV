...
plot(lmGenT)
# unclear whether DGPower is a better predictor of heat than genHour!
...
lmDGT <- lm(dT.dt ~ mean.T + mean.DGPower,
                  gw.heat.hour)
summary(lmDGT)
# Residuals:
...
#  mean.DGPower  2.045e-07  2.024e-09  101.03   <2e-16 ***
#  ---
...
# temp dropped from 44.2 to 34.5.  Mean power 690W, DGEnergy 45Wh.  On
# trendline of QQ, so this misprediction is of some "normal" type.
outl <- 2546
cat(paste("Apparent outlier: residual =", round(lmDGTH$residuals[outl],2),
...
# 45.4.  DGEnergy 58W, mean power 973W.  On trendline of QQ.
outl2 <- 4438
cat(paste("Apparent outlier: residual =", round(lmDGTH$residuals[outl2],2),
...
# 774W, DGEnergy 41W.  One of only 3 points at 4x mean residual.  Not much
# leverage so we'll leave it alone... but maybe another reason to exclude
# the first hour of every day?
#
outl3 <- 1
cat(paste("Apparent outlier: residual =", round(lmDGTH$residuals[outl3],2),
          "at", gw.heat.hour$dateTime[outl3], "in", gw.heat.hour$Source[outl3]))

lmDGTH <- lm(dT.dt ~ mean.T + mean.DGPower + mean.H.Total,
              gw.heat.hour)
summary(lmDGTH)
# Residuals:
# Min      1Q  Median      3Q     Max
...
#  mean.DGPower  2.183e-07  1.983e-09  110.08   <2e-16 ***
...
lmDGTTH <- lm(dT.dt ~ mean.T + mean.DGPower + mean.H.Total +
                  first.T.day,
               gw.heat.hour)
summary(lmDGTTH)
# Residuals:
...
#  mean.DGPower  2.480e-07  1.892e-09  131.05   <2e-16 ***
...
plot(lmDGTTH) # nonlinear; underpredicting at low and high fitted values
...
outl <- 2546
cat(paste("Apparent outlier: residual =", round(lmDGTTH$residuals[outl],2),
          "at", gw.heat.hour$dateTime[outl], "in", gw.heat.hour$Source[outl]))
gwx <- filter(gw.heat.hour, Date==gw.heat.hour$Date[outl])

gw.heat.hourm1 <- slice(gw.heat.hour,-1)
lmDGTTHm1 <- lm(dT.dt ~ mean.T + mean.DGPower + mean.H.Total +
                  first.T.day,
                gw.heat.hourm1)
summary(lmDGTTHm1)
gw.heat.hourm1 <- bind_cols(gw.heat.hourm1, lmDGTTHm1$residuals) |>
  rename(Residuals = ...32)
...
plot(lmDGTHL) # high residuals at fitted values below -5
# apparent outliers at 173, 8, 2844, 4644


gw.heat.hourx <-
  slice(gw.heat.hour,-lmDGTHL$na.action) |>
  mutate(gw.heat.hourm2, Residuals = lmDGTHL$residuals)
...
# it seems I hugely mis-estimated genHour as well as DGPower (see below).
...
# DGEnergy has a complex formula, highly nonlinear

# ouch there's a *terrible* misprediction here!
temp_modelg <- lm(dT.dt ~ Temperature. + DGPower,
                  gw.heat)
summary(temp_modelg)
# Residuals:
# Min       1Q   Median       3Q      Max
...
temp_modelh <- lm(dT.dt ~ mean.T + mean.DGPower + mean.H.Total,
                  gw.heat.hour)
summary(temp_modelh)
plot(temp_modelh)
